...
const AdminManageOrders = () => {
    ...
    return (
        <div className="admin-manage-orders-container">
            <h1>Manage Order</h1>
            <div className="admin-manage-orders-layout">
                <div className="admin-order-details">
                    <div className="admin-manage-order-details-top">
                        ...
                        <div className="admin-manage-order-details-top-right">
                ...
                <div className="admin-manage-order-product-slider">
                    <h3>Products in Order: {order.products.length}</h3>
                    <div className="admin-manage-order-slider">
                        {order.products.map((item, index) => (
                            <div className="admin-manage-order-product-card" key={index}>
                                <h4>{item.product.name}</h4>
                                <p><strong>Quantity Ordered:</strong> {item.quantity}</p>
                                <p><strong>Price:</strong> Rs.{item.priceAtPurchase}</p>
                                <p><strong>Company:</strong>{item.product.company.name}</p>
                                <p><strong>Quantity in Stock:</strong> {item.product.quantity}</p>
                            </div>
                        ))}
                    </div>
                </div>
